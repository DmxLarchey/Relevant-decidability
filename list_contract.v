...
Section occ_list_repeat.
  ...
  Proof.
    ...
    simpl.
    destruct (eqX_dec d x); lia.
    rewrite app_length in IHl; lia.
  Qed.
  ...
  Proof.
    ...
    subst; rewrite occ_perm with (1 := Hn); lia.
    generalize (Hl d).
  ...
  Proof. intros d; red; lia. Qed.

  Fact list_contract_trans l m k : l ≻c m -> m ≻c k -> l ≻c k.
  Proof. intros H1 H2 d; generalize (H1 d) (H2 d); unfold nat_contract; lia. Qed.
  ...
  Proof.
    intros H d; rewrite occ_perm with (1 := H); red; lia.
  Qed.
  ...
  Proof.
    ...
    destruct l; simpl in H; auto; lia.
  Qed.
  ...
  Proof.
    ...
    unfold nat_contract; rewrite occ_eq; simpl; lia.
  Qed.
  ...
  Proof.
    ...
    assert (occ x m = 0) as H5.
      generalize (Hlm x).
      rewrite occ_perm with (1 := H1), occ_app, occ_repeat_eq; lia.
    destruct p.
    simpl in H3.
    exfalso.
    generalize (Hk x).
    ...
    unfold nat_contract.
    destruct (eqX_dec d x) as [ | C ].
    subst d; repeat rewrite occ_repeat_eq; lia.
    repeat rewrite occ_repeat_neq; auto.
  ...
  Proof.
    ...
    generalize (H4 a); unfold nat_contract; lia.
    intros d.
  ...
  Section list_contract_isolate.
    ...
    Proof.
      ...
      rewrite occ_app, occ_repeat_eq in H2; lia.
      split.
      rewrite occ_app, occ_repeat_neq; auto.
      intro; subst y.
      rewrite occ_perm with (1 := G2) in H2.
      ...
      repeat rewrite occ_app in Hlm.
      destruct (eqX_dec d x).
      subst; repeat rewrite occ_repeat_eq in Hlm; red in Hlm |- *; lia.
      repeat (rewrite occ_repeat_neq in Hlm; auto); lia.
    ...
    Proof.
      ...
      cutrewrite (n = 2+(n-2)) in H2; try lia.
  ...
  Section list_contract_rect.
    ...
    Proof.
      ...
      repeat rewrite list_repeat_length; lia.
  ...
  Proof.
    ...
    subst; repeat rewrite occ_repeat_eq; auto; lia.
    repeat rewrite occ_repeat_neq; auto; lia.
    apply list_contract_refl.
